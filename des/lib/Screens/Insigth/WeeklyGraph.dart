...
class WeeklyGraph extends StatelessWidget {
  ...
  {
    return BlocProvider(
        create: (context) => WeeklytabsCubit(),
        child:  BlocBuilder<WeeklytabsCubit, int>(
      ...
      child:
       ...
       Column(children:
        [
            ...
            AspectsLife(),
            SizedBox(height: 20,),
            LineGraph(weeklyHistory:context.read<InsigthsCubit>().SearchAboutCategoty(context.read<WeeklyCubit>().Aspects[currentindex].aspect!) ),
        ]
        ),
    )
        );
...
class AspectsLife extends StatelessWidget {
 AspectsLife();
  ...
  Widget build(BuildContext context) { 
    ...
    return  BlocBuilder<WeeklytabsCubit, int>(
                builder: (context,currentindex)
                 {
                  return 
     Flexible(
      ...
      itemBuilder: (context, index) 
      {
       ...
       child:  Container(
        decoration: BoxDecoration(
          ...
          color:currentindex==index?  Color(0xff000000)
                                : Color(0xffF8F8FF),
        ),                    

        child:Center(child:Text(Aspects[index].aspect!,style: GoogleFonts.inter(color: Color(0xff9291A5),fontSize: 16), ), ),
           ),
          );
      }
    ),
